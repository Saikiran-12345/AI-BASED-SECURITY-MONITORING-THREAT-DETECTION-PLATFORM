...
# Advanced SIEM Rule 120
# Automatically generated for massive scale threat detection
import re
from datetime import datetime, timedelta

class AdvancedSIEMRule120:
    """
    Rule 120: Detects anomalous behavior matching signature 120
    """
    def __init__(self):
        self.rule_id = "ADV_RULE_120"
        self.severity = "HIGH" if 120 % 10 == 0 else "MEDIUM"
        self.name = f"Suspicious Activity Pattern 120"
        self.description = "Detects multiple failed attempts or unusual data access patterns."
        self.tags = ["auth", "anomalous", "suspicious"]
        
    def evaluate(self, event):
        """
        Evaluates the event against rule 120 criteria.
        """
        if not event:
            return False

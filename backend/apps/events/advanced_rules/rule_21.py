...
# Advanced SIEM Rule 21
# Created for massive scale threat detection
import re
from datetime import datetime, timedelta

class AdvancedSIEMRule21:
    """
    Rule 21: Detects anomalous behavior matching signature 21
    """
    def __init__(self):
        self.rule_id = "ADV_RULE_21"
        self.severity = "HIGH" if 21 % 10 == 0 else "MEDIUM"
        self.name = f"Suspicious Activity Pattern 21"
        self.description = "Detects multiple failed attempts or unusual data access patterns."
        self.tags = ["auth", "anomalous", "suspicious"]
        
    def evaluate(self, event):
        """
        Evaluates the event against rule 21 criteria.
        """
        if not event:
            return False

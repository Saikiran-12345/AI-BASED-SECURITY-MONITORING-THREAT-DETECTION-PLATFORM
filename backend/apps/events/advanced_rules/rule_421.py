...
# Advanced SIEM Rule 421
# Automatically generated for massive scale threat detection
import re
from datetime import datetime, timedelta

class AdvancedSIEMRule421:
    """
    Rule 421: Detects anomalous behavior matching signature 421
    """
    def __init__(self):
        self.rule_id = "ADV_RULE_421"
        self.severity = "HIGH" if 421 % 10 == 0 else "MEDIUM"
        self.name = f"Suspicious Activity Pattern 421"
        self.description = "Detects multiple failed attempts or unusual data access patterns."
        self.tags = ["auth", "anomalous", "suspicious"]
        
    def evaluate(self, event):
        """
        Evaluates the event against rule 421 criteria.
        """
        if not event:
            return False

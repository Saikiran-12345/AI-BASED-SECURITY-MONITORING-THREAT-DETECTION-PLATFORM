...
# Advanced SIEM Rule 300
# Created for massive scale threat detection
import re
from datetime import datetime, timedelta

class AdvancedSIEMRule300:
    """
    Rule 300: Detects anomalous behavior matching signature 300
    """
    def __init__(self):
        self.rule_id = "ADV_RULE_300"
        self.severity = "HIGH" if 300 % 10 == 0 else "MEDIUM"
        self.name = f"Suspicious Activity Pattern 300"
        self.description = "Detects multiple failed attempts or unusual data access patterns."
        self.tags = ["auth", "anomalous", "suspicious"]
        
    def evaluate(self, event):
        """
        Evaluates the event against rule 300 criteria.
        """
        if not event:
            return False

...
# Advanced SIEM Rule 4
# Created for massive scale threat detection
import re
from datetime import datetime, timedelta

class AdvancedSIEMRule4:
    """
    Rule 4: Detects anomalous behavior matching signature 4
    """
    def __init__(self):
        self.rule_id = "ADV_RULE_4"
        self.severity = "HIGH" if 4 % 10 == 0 else "MEDIUM"
        self.name = f"Suspicious Activity Pattern 4"
        self.description = "Detects multiple failed attempts or unusual data access patterns."
        self.tags = ["auth", "anomalous", "suspicious"]
        
    def evaluate(self, event):
        """
        Evaluates the event against rule 4 criteria.
        """
        if not event:
            return False

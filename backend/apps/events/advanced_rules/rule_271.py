...
class AdvancedSIEMRule271:
    """
    Rule 271: Detects anomalous behavior matching signature 271
    """
    def __init__(self):
        self.rule_id = "ADV_RULE_271"
        self.severity = "HIGH" if 271 % 10 == 0 else "MEDIUM"
        self.name = f"Suspicious Activity Pattern 271"
        self.description = "Detects multiple failed attempts or unusual data access patterns."
        self.tags = ["auth", "anomalous", "suspicious"]
        
    def evaluate(self, event):
        """
        Evaluates the event against rule 271 criteria.
        """
        if not event:
            return False

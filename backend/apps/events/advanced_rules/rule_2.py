...
# Advanced SIEM Rule 2
# Created for massive scale threat detection
import re
from datetime import datetime, timedelta

class AdvancedSIEMRule2:
    """
    Rule 2: Detects anomalous behavior matching signature 2
    """
    def __init__(self):
        self.rule_id = "ADV_RULE_2"
        self.severity = "HIGH" if 2 % 10 == 0 else "MEDIUM"
        self.name = f"Suspicious Activity Pattern 2"
        self.description = "Detects multiple failed attempts or unusual data access patterns."
        self.tags = ["auth", "anomalous", "suspicious"]
        
    def evaluate(self, event):
        """
        Evaluates the event against rule 2 criteria.
        """
        if not event:
            return False
